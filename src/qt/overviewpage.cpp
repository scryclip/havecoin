...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHavecoinAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeHavecoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHavecoinAmount = strAnonymizeHavecoinAmount.remove(strAnonymizeHavecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHavecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHavecoinAmount*COIN) nMaxToAnonymize = nAnonymizeHavecoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHavecoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHavecoinAmount));
        strAnonymizeHavecoinAmount = strAnonymizeHavecoinAmount.remove(strAnonymizeHavecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHavecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
        QString strMaxToAnonymize = BitcoinUnits::formatWithUnit(nDisplayUnit, nMaxToAnonymize, false, BitcoinUnits::separatorAlways);
